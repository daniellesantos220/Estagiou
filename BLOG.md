...
O sistema de blog utiliza dois perfis de usuário diferentes dos existentes. O arquivo `util/perfis.py` do projeto original contém os perfis ADMIN, ESTUDANTE e RECRUTADOR. **Você deve SUBSTITUIR os perfis ESTUDANTE e RECRUTADOR por AUTOR e LEITOR**:

### Arquivo: `util/perfis.py`

Localize o enum `Perfil` e **substitua** os perfis `ESTUDANTE` e `RECRUTADOR` por `AUTOR` e `LEITOR`. O arquivo deve ficar assim:
...
O arquivo `model/usuario_logado_model.py` contém o dataclass `UsuarioLogado` que representa o usuário autenticado. Como substituímos os perfis ESTUDANTE e RECRUTADOR, você deve **substituir os métodos `is_estudante()` e `is_recrutador()` por `is_autor()` e `is_leitor()`**:
...
> **Nota:** O método `is_admin()` deve ser mantido. Remova apenas `is_estudante()` e `is_recrutador()`.
