...
Estudante de teste com sessão limpa para cada teste.
...
Dicionário com dados de um usuário de teste (Estudante).

```python
def test_com_dados_usuario(usuario_teste):
    assert usuario_teste["email"] == "teste@example.com"
    assert usuario_teste["perfil"] == Perfil.ESTUDANTE.value
```
...
#### `recrutador_teste` - Dados de recrutador
Dicionário com dados de um recrutador de teste.

```python
def test_com_dados_recrutador(recrutador_teste):
    assert recrutador_teste["perfil"] == Perfil.RECRUTADOR.value
```
...
### Fixtures de Estudante Autenticado

#### `estudante_autenticado` - Estudante logado como usuário
Estudante TestClient já autenticado como usuário padrão (Estudante).

```python
def test_acessar_dashboard(estudante_autenticado):
    response = estudante_autenticado.get("/usuario")
    assert response.status_code == 200
```

#### `admin_autenticado` - Estudante logado como admin
Estudante TestClient já autenticado como administrador.
...
#### `recrutador_autenticado` - Estudante logado como recrutador
Estudante TestClient já autenticado como recrutador.

```python
def test_acessar_vendas(recrutador_autenticado):
    response = recrutador_autenticado.get("/vendas")
    assert response.status_code == 200
```
...
#### `usuario_com_foto` - Estudante com foto de perfil
Estudante autenticado que já tem foto de perfil salva.
...
```python
def test_upload_foto(estudante_autenticado, foto_teste_base64):
    response = estudante_autenticado.post("/perfil/atualizar-foto", json={
...
def test_dashboard_exibe_nome(estudante_autenticado, usuario_teste):
    response = estudante_autenticado.get("/usuario")
...
def test_fluxo_completo_cadastro_e_login(client):
    ...
    assert usuario.perfil == Perfil.ESTUDANTE.value
...
```python
def test_estudante_nao_acessa_area_admin(estudante_autenticado):
    """Estudante não deve ter acesso a áreas administrativas."""

    # Tentar acessar listagem de usuários (admin only)
    response = estudante_autenticado.get("/admin/usuarios/listar", follow_redirects=False)
