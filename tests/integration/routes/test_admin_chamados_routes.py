...
class TestAdminChamadosAutorizacao:
    ...
    def test_listar_requer_admin(self, estudante_autenticado):
        """Apenas admin pode listar todos os chamados"""
        response = estudante_autenticado.get(
            "/admin/chamados/listar", follow_redirects=False
        )
        # Estudante comum deve ser negado (403 ou redirect)
        assert response.status_code in [
            status.HTTP_303_SEE_OTHER,
            status.HTTP_403_FORBIDDEN,
        ]

    def test_responder_get_requer_admin(self, estudante_autenticado):
    ...
    def test_fechar_requer_admin(self, estudante_autenticado):
    ...
    def test_reabrir_requer_admin(self, estudante_autenticado):
