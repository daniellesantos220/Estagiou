...
class TestTema:
    ...
    def test_get_tema_requer_admin(self, estudante_autenticado):
        """Estudante não deve acessar seletor de temas"""
        response = estudante_autenticado.get("/admin/tema", follow_redirects=False)
    ...
    def test_estudante_nao_pode_aplicar_tema(self, estudante_autenticado):
        """Estudante não deve poder aplicar tema"""
...
class TestAuditoria:
    ...
    def test_get_auditoria_requer_admin(self, estudante_autenticado):
        """Estudante não deve acessar auditoria"""
        response = estudante_autenticado.get("/admin/auditoria", follow_redirects=False)
        assert response.status_code in [
            status.HTTP_303_SEE_OTHER,
            status.HTTP_403_FORBIDDEN,
        ]
    ...
    def test_estudante_nao_pode_filtrar_logs(self, estudante_autenticado):
        """Estudante não deve poder filtrar logs"""
        data_hoje = agora().strftime("%Y-%m-%d")
...
class TestSegurancaConfiguracoes:
    ...
    def test_recrutador_nao_acessa_tema(self, recrutador_autenticado):
        """Recrutador não deve acessar temas"""
        response = recrutador_autenticado.get("/admin/tema", follow_redirects=False)
        assert response.status_code in [
            status.HTTP_303_SEE_OTHER,
            status.HTTP_403_FORBIDDEN,
        ]

    def test_recrutador_nao_acessa_auditoria(self, recrutador_autenticado):
        """Recrutador não deve acessar auditoria"""
        response = recrutador_autenticado.get(
            "/admin/auditoria", follow_redirects=False
        )
        assert response.status_code in [
            status.HTTP_303_SEE_OTHER,
            status.HTTP_403_FORBIDDEN,
