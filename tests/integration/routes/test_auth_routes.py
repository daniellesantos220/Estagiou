...
class TestLogin:
    ...
    def test_usuario_logado_nao_acessa_login(self, estudante_autenticado):
        """Usuário já logado deve ser redirecionado ao acessar /login"""
        response = estudante_autenticado.get("/login", follow_redirects=False)
        assert_redirects_to(response, "/usuario")
...
class TestCadastro:
    ...
    def test_cadastro_cria_usuario_com_perfil_estudante(self, client):
        """Cadastro público deve criar usuário com perfil ESTUDANTE (Enum Perfil)"""
        ...
        assert usuario.perfil == Perfil.ESTUDANTE.value  # Usa Enum Perfil


class TestLogout:
    """Testes de logout"""

    def test_logout_limpa_sessao(self, estudante_autenticado):
        """Logout deve limpar sessão e redirecionar para raiz"""
        response = estudante_autenticado.get("/logout", follow_redirects=False)

        # Deve redirecionar para raiz do site
        assert_redirects_to(response, "/")

    def test_logout_desautentica_usuario(self, estudante_autenticado):
        """Após logout, usuário não deve ter acesso a áreas protegidas"""
        # Fazer logout
        estudante_autenticado.get("/logout")

        # Tentar acessar área protegida
        response = estudante_autenticado.get("/chamados/listar", follow_redirects=False)
...
class TestAutorizacao:
    ...
    def test_usuario_autenticado_acessa_area_protegida(self, estudante_autenticado):
        """Usuário autenticado deve acessar áreas protegidas"""
        response = estudante_autenticado.get("/chamados/listar")
        assert response.status_code == status.HTTP_200_OK

    def test_estudante_nao_acessa_area_admin(self, estudante_autenticado):
        """Estudante não deve acessar áreas administrativas"""
...
class TestCadastroAdicional:
    ...
    def test_usuario_logado_nao_acessa_cadastro(self, estudante_autenticado):
        """Usuário já logado deve ser redirecionado ao acessar /cadastrar"""
        response = estudante_autenticado.get("/cadastrar", follow_redirects=False)
        assert_redirects_to(response, "/usuario")
