...
class TestRotasPublicasComUsuarioLogado:
    ...
    def test_home_acessivel_usuario_logado(self, estudante_autenticado):
        """Usuário logado também pode acessar landing page"""
        response = estudante_autenticado.get("/")
        assert response.status_code == status.HTTP_200_OK

    def test_sobre_acessivel_usuario_logado(self, estudante_autenticado):
        """Usuário logado pode acessar página sobre"""
        response = estudante_autenticado.get("/sobre")
        assert response.status_code == status.HTTP_200_OK
